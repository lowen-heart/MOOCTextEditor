...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		WPTreeNode word = new WPTreeNode(word1,null);
		...
		WPTreeNode curr;
		WPTreeNode newnode;
		List<String> disonewords;
		
		while(!queue.isEmpty()){
			curr = queue.remove();
			disonewords = nw.distanceOne(curr.getWord(), true);
			for(String n : disonewords){
				if(!visited.contains(n)){
					newnode = curr.addChild(n);
					visited.add(n);
					queue.add(newnode);
					if(n.equals(word2)){
